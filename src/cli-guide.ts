...
const COMMAND_GUIDES: CommandGuide[] = [
  {
    ...
    nextSteps: ['Run `build` once the folders look right.'],
    examples: ['doctor'],
  },
  {
    name: 'validate',
    description: 'Check configs, patches, and scripts without writing a preview.',
    purpose:
      'Reads your configs, applies your patches in memory, runs your script tests, and reports anything broken. Game files and preview output stay unchanged; scripts may update their source data and caches.',
    nextSteps: [
  ...
  },
  {
    ...
    purpose:
      'Rebuilds from current source and applies it to your WARNO mod folder. Saves originals for recovery; compile the mod with WARNO tools afterwards.',
    resultLabel: 'Game mod',
    resultPath: DEFAULT_GAME_PATH,
    nextSteps: [
      'Compile the mod with WARNO tools, then test it in game.',
      'Run `recover --yes` to undo this later.',
    ],
  ...
  },
  {
    ...
    nextSteps: ['Run `build --mod <id>` and read what it produced.'],
...
export function buildRootHelpText(): string {
  const flowGuides = ['doctor', 'build', 'sync'].map((name) =>
  ...
  return [
    ...
    ...orderedFlow,
    '',
    '  Build includes validation. After sync, compile with WARNO tools.',
    '',
    'Where things live:',
    ...formatFactLines(
      [
