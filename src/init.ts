...
This source mod was created by \`bun run ymb init\` and is meant to be a safe, beginner-friendly starting point.
...
1. Run \`bun run ymb validate --mod ${answers.id}\`.
2. Run \`bun run ymb build --mod ${answers.id}\` to preview the starter output.
...
function renderDemoScript(): string {
  return `interface StarterBuildInfoContext {
...
interface GeneratedStarterBuildInfoFile {
  targetRelativePath: string;
  content: string;
}

export default async function generateBuildInfo(
  context: StarterBuildInfoContext,
): Promise<GeneratedStarterBuildInfoFile> {
  const targetRelativePath = readRequiredStringVariable(context.variables, 'generatedInfoTarget');
  const description =
    context.mod.config.description || 'Starter scaffold generated by YMB.';
  const blockName = buildStarterBlockName(context.mod.config.id);

  return {
    targetRelativePath,
    content: \`\${blockName} is TGeneratedModInfo
(
    ModId = \${JSON.stringify(context.mod.config.id)}
    ModName = \${JSON.stringify(context.mod.config.name)}
...
function renderDemoScriptTest(answers: InitAnswers): string {
  ...
  return [
    ...
    '    };',
    '',
    'export default async function testGenerateBuildInfo(): Promise<{ results: ScriptTestResult[] }> {',
    '  const generated = (await generateBuildInfo({',
    '    variables: {',
    `      generatedInfoTarget: '${expectedTarget}',`,
    '    },',
    '    mod: {',
    '      config: {',
    `        id: '${answers.id}',`,
    `        name: ${JSON.stringify(answers.name)},`,
    `        description: ${answers.description ? JSON.stringify(answers.description) : 'undefined'},`,
    '      },',
    '    },',
    '  })) as GeneratedStarterBuildInfoFile;',
    '',
    '  return {',
    '    results: [',
