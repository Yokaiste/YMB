...
async function collectInitAnswers(options: InitCommandOptions): Promise<InitAnswers> {
  ...
  // A description is genuinely optional, so leaving it out is not a reason to
  // ask for one. Counting it as missing meant `init --id x --name Y` - the form
  // the help, the docs, and the failure's own suggestion all print - dropped
  // into an interactive prompt, and failed outright wherever there is no
