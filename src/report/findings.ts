...
export function formatFindingGroups(findings: readonly ReportFinding[]): string[] {
  ...
  for (const group of groupBy(findings, (finding) => [
    ...
    if (byDetail.length === 1) {
      lines.push(`${heading} ${[first.detail, first.suggestion].filter(Boolean).join(' ')}`);
      lines.push(...formatMembers(group, MEMBER_INDENT));
      continue;
    }

    lines.push(`${heading}${first.suggestion ? ` ${first.suggestion}` : ''}`);
    for (const subgroup of byDetail) {
      const leader = subgroup[0];
      if (!leader) continue;
      lines.push(`${MEMBER_INDENT}${subgroup.length}x ${leader.detail}`);
      lines.push(...formatMembers(subgroup, SUBGROUP_MEMBER_INDENT));
    }
  }
  return lines;
}
...
function groupBy(
  ...
  const groups = Map.groupBy(findings, (finding) => JSON.stringify(toKeyParts(finding)));
...
const MEMBER_ORDER = new Intl.Collator(undefined, { numeric: true });

function sortMembers(findings: readonly ReportFinding[]): ReportFinding[] {
  return [...findings].sort(
    (left, right) =>
      MEMBER_ORDER.compare(left.subject, right.subject) ||
      MEMBER_ORDER.compare(left.origin ?? '', right.origin ?? '') ||
      MEMBER_ORDER.compare(left.targetFile ?? '', right.targetFile ?? '') ||
      MEMBER_ORDER.compare(left.target ?? '', right.target ?? ''),
  );
}
...
function formatMembers(findings: readonly ReportFinding[], indent: string): string[] {
  const groups = Map.groupBy(sortMembers(findings), (finding) =>
    JSON.stringify([finding.subject, finding.origin, finding.targetFile]),
  );
  const lines: string[] = [];
  for (const members of groups.values()) {
    const first = members[0];
    if (!first) continue;
    const targets = Map.groupBy(members, (member) => member.target ?? '');
    const hasTargets = [...targets.keys()].some(Boolean);
    lines.push(
      `${indent}${describeMember(first)}${!hasTargets && members.length > 1 ? ` (${members.length} occurrences)` : ''}`,
    );
    if (first.targetFile) lines.push(`${indent}  ${first.targetFile}`);
    if (hasTargets) {
      for (const [target, entries] of targets) {
        lines.push(
          `${indent}    ${target || '(target unavailable)'}${entries.length > 1 ? ` (${entries.length} occurrences)` : ''}`,
        );
      }
    }
  }
