...
import { parseNdfScalar } from './values.ts';
...
export function applyBulkOperation(
  ...
  const groups = buffer
    ...
    .map((block) => ({
      block,
      matches: (operation.recursive
        ? findObjectBlocks(block.text)
        : [{ ...block, start: 0, end: block.text.length }]
      ).filter((node) => matchesBlock(node, operation.match, unreached)),
    }))
    .filter((group) => group.matches.length > 0);
  const matchedBlockCount = groups.reduce((count, group) => count + group.matches.length, 0);
  reportUnreachedConditionValues(unreached, operation, context, onNotice);

  ensure(matchedBlockCount >= operation.expect.minBlocks, 'SelectorError', {
    ...errorOwner(context),
    reason: `Bulk operation matched ${matchedBlockCount} block(s) but expects at least ${operation.expect.minBlocks}.`,
    suggestion:
  ...
  const rewrites = groups.flatMap(({ block, matches }) => {
    let cursor = 0;
    const parts: string[] = [];
    for (const node of matches) {
      ensure(node.start >= cursor, 'SelectorError', {
        ...errorOwner(context),
        reason: 'Recursive bulk conditions matched overlapping objects.',
        suggestion: 'Select a specific object type so a parent and its child are not edited twice.',
      });
      parts.push(block.text.slice(cursor, node.start));
      parts.push(
        rewriteBlock(node.text, operation, changesPerEdit, alreadySetPerEdit, context) ?? node.text,
      );
      cursor = node.end;
    }
    ...
    assertEditMetItsExpectation(
      {
        ...
        matchedBlocks: matchedBlockCount,
      },
      context,
      onNotice,
    );
...
function createUnreachedValueTracker(operation: BulkOperation): UnreachedValues {
  ...
  return operation.match.conditions.map((condition) =>
    ...
    condition.is === 'notContains' || condition.is === 'notEquals'
      ? new Set<string>()
...
export function matchesBlock(
  ...
  if (!unreached) {
    const test = (condition: BulkCondition) => matchesCondition(block, condition, undefined);
    return match.mode === 'any' ? match.conditions.some(test) : match.conditions.every(test);
  }
...
function matchesCondition(
  ...
  const rawSubject = readConditionSubject(block, condition);
  const subject = rawSubject ?? '';
  if (condition.is === 'equals' || condition.is === 'notEquals') {
    if (rawSubject === undefined) return false;
    const actual =
      condition.on === 'field' ? parseNdfScalar(stripLineComments(subject)).value : subject;
    let matched = false;
    for (const value of condition.value) {
      const expected =
        condition.on === 'field' ? parseNdfScalar(String(value)).value : String(value);
      if (actual !== expected) continue;
      matched = true;
      unreached?.delete(value);
      if (!unreached || unreached.size === 0) break;
    }
    return condition.is === 'equals' ? matched : !matched;
  }
...
function satisfiesConditionValue(
  subject: string,
  is: Exclude<BulkCondition['is'], 'notContains' | 'equals' | 'notEquals'>,
  value: string,
): boolean {
  switch (is) {
    case 'startsWith':
...
function readConditionSubject(block: TopLevelBlock, condition: BulkCondition): string | undefined {
  switch (condition.on) {
    case 'name':
      return block.name ?? '';
    case 'type':
