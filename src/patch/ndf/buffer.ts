...
export class NdfTextBuffer {
  ...
  constructor(baseText: string) {
    ...
    this.blockList = findTopLevelBlocks(baseText);
  ...
  includes(needle: string): boolean {
    ...
    for (const piece of this.pieces) {
      ...
      if (seamReach > 0 && `${suffix}${piece.slice(0, seamReach)}`.includes(needle)) {
        return true;
      }
      if (seamReach > 0) suffix = `${suffix}${piece.slice(-seamReach)}`.slice(-seamReach);
    }
    return false;
  }
