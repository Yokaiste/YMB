...
/** Numeric exports include enum ordinals, which can differ between game-data sources. */
export function readNumericDefinitions(text: string): Record<string, number> {
  const definitions: Record<string, number> = Object.create(null);
  const pattern =
    /(?:export\s+)?([A-Za-z_][A-Za-z0-9_]*)\s+is\s+([+-]?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?)(?=[ \t]*(?:\/\/|\r?\n|$))/y;
  const state = createNdfLexicalState();
  let depth = 0;
  for (let index = 0; index < text.length; index += 1) {
    const lexical = advanceNdfLexicalState(state, text, index);
    if (lexical === 'comment-start') index += 1;
    if (lexical !== 'code') continue;
    if (depth === 0 && (index === 0 || isWhitespaceCode(text.charCodeAt(index - 1)))) {
      pattern.lastIndex = index;
      const match = pattern.exec(text);
      if (match?.[1] && match[2] && Number.isFinite(Number(match[2]))) {
        definitions[match[1]] = Number(match[2]);
        index = pattern.lastIndex - 1;
        continue;
      }
    }
    const code = text.charCodeAt(index);
    if (isOpeningDelimiter(code)) depth += 1;
    else if (isClosingDelimiter(code)) depth -= 1;
  }
  return definitions;
}
 ...
 */
/** Enumerate typed objects once, including anonymous objects in fields and collections. */
export function findObjectBlocks(text: string): TopLevelBlock[] {
  const roots = new Map(findTopLevelBlocks(text).map((block) => [block.end, block]));
  const objects: TopLevelBlock[] = [];
  const stack: Array<{ start: number; typeName: string } | undefined> = [];
  const state = createNdfLexicalState();
  let identifier: { start: number; typeName: string } | undefined;
  for (let index = 0; index < text.length; index += 1) {
    const lexical = advanceNdfLexicalState(state, text, index);
    if (lexical === 'comment-start') index += 1;
    if (state.inString) identifier = undefined;
    if (lexical !== 'code') continue;
    if (isIdentifierCode(text.charCodeAt(index))) {
      const start = index;
      while (index + 1 < text.length && isIdentifierCode(text.charCodeAt(index + 1))) index += 1;
      const typeName = text.slice(start, index + 1);
      identifier = /^[A-Za-z_]/.test(typeName) ? { start, typeName } : undefined;
      continue;
    }
    if (text[index] === '(') {
      ...
      const root = roots.get(index + 1);
      if (root) objects.push(root);
      else if (header)
        objects.push({ ...header, end: index + 1, text: text.slice(header.start, index + 1) });
    }
    if (!isWhitespaceCode(text.charCodeAt(index))) identifier = undefined;
  }
  return objects.sort((left, right) => left.start - right.start);
}
