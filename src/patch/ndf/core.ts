...
import { NdfTextBuffer } from './buffer.ts';
...
import { reidentifyDescriptorIds } from './identity.ts';
import {
  createPatchNotice,
  describeNdfValue,
...
export async function applyPatchTargetCooperative(
  ...
  const buffer = new NdfTextBuffer(currentText);
    ...
    applyOperation(
      ...
      watched.onNotice
        ? (notice) =>
            watched.onNotice?.({
              ...notice,
              target: `${operation.op} ${operation.destination?.name ?? operation.selector?.value ?? `operation #${operationIndex + 1}`}`,
            })
        : undefined,
    );
  }
...
function applyCopy(
  ...
  let copiedText = sourceBlock.text.replace(
    new RegExp(`\\b${RegExp.escape(sourceBlock.name)}\\b`, 'g'),
    () => destinationName,
  );
  if (operation.reidentify) {
    const targetPath =
      absolutePath.replaceAll('\\', '/').match(/(?:^|\/)((?:GameData|CommonData)\/.*)$/)?.[1] ??
      absolutePath;
    copiedText = reidentifyDescriptorIds(
      copiedText,
      JSON.stringify([
        application.mod.config.id,
        application.patch.config.id,
        targetPath,
        destinationName,
      ]),
    );
...
function endsWithCollectionSeparator(text: string): boolean {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    ...
    const code = stripLineComments(trimmed).trimEnd();
    if (code.length === 0) {
      continue;
    }
...
export function insertCollectionEntryByPath(
  ...
  const buffer = new NdfTextBuffer(text);
...
function normalizeCollectionEntrySnippet(value: string): string {
  ...
  if (trimmedEnd.length === 0 || endsWithCollectionSeparator(trimmedEnd)) {
    return trimmedEnd;
  }
  return `${trimmedEnd},`;
}
