...
function applyCopy(
  ...
  const copiedText = sourceBlock.text.replace(
    new RegExp(`\\b${escapeRegExp(sourceBlock.name ?? '')}\\b`),
...
function updateCollectionValue(
  ...
  const rawRenderedEntry = renderCollectionEntryValue(
    nextValue,
    application,
    absolutePath,
    operationIndex,
  ...
  if (collectionAlreadyContainsEntry(collectionValue, rawRenderedEntry, normalizedRenderedEntry)) {
    return currentValue;
  }
  ...
  const renderedEntry = wrapAddedSnippetWithMarkers(
...
function collectionAlreadyContainsEntry(
  ...
  normalizedRenderedEntry: string,
): boolean {
  return [normalizedRenderedEntry, rawRenderedEntry]
    .map((entry) => entry.trim())
    .filter((entry, index, entries) => entry.length > 0 && entries.indexOf(entry) === index)
    .some((entry) => collectionValue.includes(entry));
}
