...
import {
  ...
  primeTopLevelBlockIndex,
  registerTopLevelTextEdit,
  splitNdfPath,
} from './scan.ts';
import {
  advanceStringState,
...
export function applyPatchTarget(
  ...
  if (target.operations.length > 1) {
    primeTopLevelBlockIndex(nextText);
  }
...
function applyOperation(
  ...
  switch (operation.op) {
    case 'bulk':
      return applyBulkOperation(currentText, operation, application, absolutePath, operationIndex);
    case 'modify':
...
function replaceTopLevelRange(
  text: string,
  start: number,
  end: number,
  replacement: string,
): string {
  const updatedText = replaceRange(text, start, end, replacement);
  registerTopLevelTextEdit(text, updatedText, start, end, replacement);
  return updatedText;
}
...
function applyModify(
  ...
  if (operation.selector.kind === 'field' && operation.selector.by === 'path') {
    ...
    const segments = splitNdfPath(operation.selector.value);
...
function applyCopy(
  ...
  return replaceTopLevelRange(currentText, currentText.length, currentText.length, insertion);
}
...
function updateFieldPath(
  ...
  return replaceTopLevelRange(currentText, exportBlock.start, exportBlock.end, updatedBlockText);
}
...
function resolveTopLevelBlockReference(
  ...
  if (reference.startsWith('@type:')) {
    ...
    const matches = findTopLevelBlocks(currentText).filter(
      (block) => block.typeName.trim().split(/\s+/u)[0] === requestedType,
    );
    ensure(matches.length === 1, 'SelectorError', {
      ...identity,
      reason:
        matches.length === 0
          ? `Top-level type selector \`${reference}\` matched no blocks.`
          : `Top-level type selector \`${reference}\` matched multiple blocks.`,
...
export function insertCollectionEntryByPath(
  ...
  const updatedText = replaceTopLevelRange(text, block.start, block.end, updatedBlockText);
