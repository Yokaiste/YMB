  ...
  while (pendingIndex < pending.length) {
    ...
    for (const dependencyModId of current.config.dependsOn) {
      ...
      const dependency = discoveredById.get(dependencyModId);
        ...
        absolutePath: current.configFilePath,
        ...
        details: [dependency.configFilePath],
      });
...
export async function resolvePatchDependenciesCooperative(
  ...
  while (dependencyCursor < selectedPatches.length) {
    await yieldController?.maybeYield();
    const application = selectedPatches[dependencyCursor];
    dependencyCursor += 1;
    if (!application) {
      continue;
    }
  ...
  const selectedModsWithPatchOwners = new Map(
    selectedMods.map((mod) => [mod.config.id, mod] as const),
  );
  for (const selectedPatch of selectedPatches) {
    selectedModsWithPatchOwners.set(selectedPatch.mod.config.id, selectedPatch.mod);
  }
  const orderedSelectedMods = await resolveSelectedModsCooperative(
    [...selectedModsWithPatchOwners.values()],
    discoveredMods,
    yieldController,
  );
  selectedMods.splice(0, selectedMods.length, ...orderedSelectedMods);
  ...
  while (queue.length > 0) {
    ...
    absolutePath: selectedPatches[0]?.mod.configFilePath ?? '<selection>',
  ...
  for (const selectedPatch of selectedPatches) {
    ...
    const selectedMod = selectedModsById.get(selectedPatch.mod.config.id);
      ...
      absolutePath: selectedPatch.patch.configFilePath,
...
function ensureDependencyEnabled(
  ...
  dependencyReference: string,
    ...
    absolutePath: owner.patch.configFilePath,
    ...
    details: [dependency.patch.configFilePath],
  });
}
...
function topologicallyOrderMods(selectedMods: DiscoveredMod[]): DiscoveredMod[] {
  ...
  while (queue.length > 0) {
    ...
    absolutePath: selectedMods[0]?.configFilePath ?? '<selection>',
...
function resolvePatchDependencyReference(
  ...
  const dependencyOptions = indexByPatchId.get(dependencyReference) ?? [];
    ...
    details: dependencyOptions.map((option) => option.patch.configFilePath),
  });
...
function validateDependencyModPriority(
  ...
  dependency: PatchApplication,
    ...
    absolutePath: selected.patch.configFilePath,
