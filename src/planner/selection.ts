...
import { isScopeIncluded, listFilesRecursive } from '../config/layout.ts';
import { ensure } from '../errors.ts';
import { assertOwnedRelativePath, normalizeRelativePath, toPathKey } from '../path-utils.ts';
import { claimSelectionIdentity, matchesAnySelectionFilter } from '../selection-filter.ts';
...
export async function selectPatchesCooperative(
  ...
  for (const mod of discoveredMods) {
    ...
    for (const patch of mod.patches) {
      ...
      const duplicateOwner = claimSelectionIdentity(
        patchIds,
        patch.config.id,
        patch.configFilePath,
      );
  ...
  return config.tests.map((test) => ({
    absolutePath: path.join(
      ...
      ...assertOwnedRelativePath(
        String(resolveTemplateValue(test.path, templateVariables)),
        ownerRoot,
        ownerLabel,
      ).split('/'),
    ),
...
export async function collectTargetFilesCooperative(
  ...
  for (const deletion of selectedFileDeletions) {
    await yieldController?.maybeYield();
    targetFiles.set(toPathKey(deletion.targetRelativePath), deletion.targetRelativePath);
  }
