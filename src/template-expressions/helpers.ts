...
/**
 * Every helper takes the evaluated argument list as-is and narrows what it needs,
 * so the table below can be called by name with no per-helper wiring.
 */
const expressionHelpers = {
  ...
  concat(...values: unknown[]): unknown[] {
    const arrays = values.map((value, index) =>
      coerceArray(value, `concat() argument ${index + 1}`),
    );
    assertGeneratedItemCount(
      arrays.reduce((count, value) => count + value.length, 0),
      'concat()',
    );
    return arrays.flat();
  },
  pluck(value: unknown, key: unknown): unknown[] {
    if (typeof key !== 'string') throw new Error('`pluck()` expects a field name.');
    return coerceArray(value, 'pluck() value').map((entry, index) => {
      if (
        !entry ||
        typeof entry !== 'object' ||
        Array.isArray(entry) ||
        ...
        throw new Error(`\`pluck()\` entry ${index + 1} has no field "${key}".`);
      }
  ...
  range(...args: unknown[]): number[] {
    ...
    if (![start, end, step].every(Number.isSafeInteger)) {
      throw new Error('`range()` arguments must be safe integers.');
    }
...
} satisfies Record<ExpressionHelperName, (...args: unknown[]) => unknown>;

// Typed once here so a missing or misnamed helper is a compile error, and adding
// one means editing `ExpressionHelperName` and the table above - nothing else.
const helpersByName: Record<ExpressionHelperName, (...args: unknown[]) => unknown> =
  expressionHelpers;

export function isExpressionHelperName(name: string): name is ExpressionHelperName {
  return Object.hasOwn(helpersByName, name);
}

export function invokeExpressionHelper(callee: ExpressionHelperName, args: unknown[]): unknown {
  return helpersByName[callee](...args);
}
