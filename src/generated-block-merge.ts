...
interface GeneratedBlockSnapshot {
  strippedText: string;
  blocks: Map<string, ParsedGeneratedBlock>;
  duplicateIds: string[];
}
...
export function tryMergeGeneratedBlocks(
  currentText: string,
  nextText: string,
  ownerPaths: string | readonly string[],
): GeneratedBlockMergeResult {
  const allowedOwnerPaths = typeof ownerPaths === 'string' ? [ownerPaths] : ownerPaths;
  ...
  const duplicateIds = [
    ...new Set([...currentSnapshot.duplicateIds, ...nextSnapshot.duplicateIds]),
  ];
  if (duplicateIds.length > 0) {
    return {
      kind: 'conflict',
      reason: 'Generated block ids must be unique within one target.',
      details: duplicateIds.map((id) => `Duplicate generated block: ${id}`),
    };
  }
...
function parseGeneratedBlocks(content: string): GeneratedBlockSnapshot | undefined {
  ...
  for (const range of ranges) {
    ...
    lastIndex = range.end;
    if (blocks.has(range.id)) {
      duplicateIds.add(range.id);
    }
    blocks.set(range.id, {
