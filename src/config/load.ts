...
export async function loadBuilderProjectConfig(filePath: string): Promise<BuilderProjectConfig> {
  ...
  assertConfigTree(filePath, document.value);
...
function parseConfig<T>(
  ...
  assertConfigTree(filePath, value);
...
/** Reject cycles before schema recursion and keys Zod would silently discard. */
function assertConfigTree(filePath: string, value: unknown): void {
  ...
  const visit = (current: unknown, path: Array<string | number>): void => {
    ...
    if (ancestors.has(current)) {
      throw new YmbError('ConfigError', {
        absolutePath: filePath,
        reason: `Config contains a circular YAML alias at \`${path.join('.')}\`.`,
        suggestion:
          'Remove the alias that refers to its own parent; shared non-circular aliases are supported.',
      });
    }
