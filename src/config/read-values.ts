...
import { NdfTextBuffer } from '../patch/ndf/buffer.ts';
import { readNestedPathValue, readNumericDefinitions, splitNdfPath } from '../patch/ndf/scan.ts';
...
async function readOneValue(
  ...
  if (blockReference && !blockReference.startsWith('@type:') && fieldSegments.length === 0) {
    const value = readNumericDefinitions(content)[blockReference];
    ensure(value !== undefined, 'ConfigError', {
      ...owner,
      reason: `\`readValues.${name}\` found no numeric definition \`${blockReference}\` in \`${relativePath}\`.`,
      suggestion: 'Use the name of a numeric definition, or a block and field path.',
    });
    return value;
  }
  ...
  const block = resolveTopLevelBlockReference(new NdfTextBuffer(content), blockReference, {
    absolutePath,
    modId: owner.modId,
