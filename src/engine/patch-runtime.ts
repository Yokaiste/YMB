...
import {
  ...
  runIpcWorker,
} from '../scripts/runtime-shared.ts';
...
type PatchRuntimeResponse = RuntimeResponse<{
  writtenFile: Exchanged<WrittenBuildFile>;
...
export function serializePatchRuntimeError(error: unknown): RuntimeErrorPayload {
...
function isPatchRuntimeResponse(message: unknown): message is PatchRuntimeResponse {
  return isRuntimeResponse(
    message,
    (candidate) => typeof candidate.writtenFile === 'object' && candidate.writtenFile !== null,
  );
}
