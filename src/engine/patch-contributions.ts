...
import { applyPatchTargetCooperative, validateNdf, validateNdfCooperative } from '../patch/ndf.ts';
...
export async function applyContributionSequence(
  ...
  if (yieldController) {
    await validateNdfCooperative(updatedText, absolutePath, yieldController);
  } else {
    validateNdf(updatedText, absolutePath);
  }
  return updatedText;
}
...
export async function buildPatchPriorityContributions(
  ...
  for (const modId of sortedModIds) {
    ...
    previews.push({
      ...
      previewContent: await applyContributionSequence(
        baseText,
