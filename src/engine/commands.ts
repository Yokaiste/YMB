...
import { type CommandOutputLines, selectionCommand, toCommandOutput } from '../report/output.ts';
...
import { preparePlanForContext, withOptionalPatchesResolved } from './plan.ts';
...
export async function runValidate(
  ...
  return withBuilderOperationLock(builderPath, 'validate', (context) =>
    runValidateUnlocked(context, selection),
  );
}
...
async function runValidateUnlocked(
  ...
  const preparedPlan = await preparePlanForContext(context, selection);
  ...
  return toCommandOutput(result.logs, {
    ...
    data: {
      ...describePlanData(plan),
      counts: {
        patchTargets: result.validatedPatchTargets,
        replaceTargets: result.validatedReplaceTargets,
        fileOperationTargets: result.validatedFileOperationTargets,
        scriptTargets: result.validatedScriptTargets,
        scriptTests: result.validatedScriptTests,
    ...
    nextSteps: [
      `Run \`${selectionCommand('build', selection)}\` to write a preview.`,
...
async function checkPlan(plan: BuildPlan) {
  ...
  warningCount += recordNotices(plan.notices, patchNotices) + plan.unmatchedFilters.length;
  ...
  for (const writtenFile of materializedFiles) {
    warningCount += recordNotices(writtenFile.notices, patchNotices);
  }
...
export async function runBuild(
  ...
  return withBuilderOperationLock(builderPath, 'build', (context) =>
    runBuildUnlocked(context, selection),
  );
}
...
async function runBuildUnlocked(
  ...
  try {
    ...
    if (stagedBuildOutputRoot) {
      await rm(stagedBuildOutputRoot, { recursive: true, force: true }).catch(() => undefined);
    }
    throw error;
  }
  ...
  return toCommandOutput(logs, {
    ...
    data: {
      ...describePlanData(plan),
      files: writtenFiles.map(({ targetRelativePath, sourceType, contributors }) => ({
      ...
      counts: {
        ...
        scriptTests: executedScriptTests,
      },
      notices: patchNotices,
      findings,
      cache: materializationMetrics,
      durationMs: finishedAt - startedAt,
    },
    detailHeading: 'preview files',
    locations: [{ label: 'preview', path: buildOutputRoot }],
    nextSteps: selection.dryRun
      ? [`Run \`${selectionCommand('build', selection)}\` to write the preview files.`]
      : [
          'Open the preview folder and inspect the files you changed.',
          `Review the preview, then run \`${selectionCommand('sync', selection, ['--yes'])}\`.`,
        ],
  });
}
...
export async function runSync(
  ...
  return withBuilderOperationLock(builderPath, 'sync', (context) =>
    runSyncUnlocked(context, selection),
  );
}
...
async function runSyncUnlocked(
  ...
  let plan = await preparePlanForContext(context, selection);
  const afterPlan = performance.now();
  const logs: string[] = [];
  ...
  return toCommandOutput(logs, {
    ...
    nextSteps: selection.dryRun
      ? [`Run \`${selectionCommand('sync', selection, ['--yes'])}\` if the plan looks correct.`]
      : [
          'Compile the mod with WARNO tools, then test it in game.',
...
function describeSkippedPatches(plan: BuildPlan): string[] {
  return formatFindingGroups(
    plan.skippedPatches.map((skipped) => ({
      severity: 'note',
      label: 'skipped optional patch',
      subject: skipped.patchId,
...
function describePlanData(plan: BuildPlan): Record<string, unknown> {
  return {
    mods: plan.selectedMods.map((mod) => mod.config.id),
    patches: plan.selectedPatches.map(({ mod, patch }) => ({
      modId: mod.config.id,
      patchId: patch.config.id,
    })),
    skippedPatches: plan.skippedPatches,
    unmatchedFilters: plan.unmatchedFilters,
    deletions: plan.selectedFileDeletions.map((entry) => entry.targetRelativePath),
  };
}
