...
// Rebuilt releases may keep their version; fingerprint the actual code and runtime.
export function getCacheSalt(): string {
  cacheSalt ??= `${CACHE_SCHEMA_VERSION}:${packageDefinition.version}:${Bun.version}:${resolveBuilderSourceDigest()}`;
  return cacheSalt;
}

function resolveBuilderSourceDigest(): string {
  try {
    if (!import.meta.url.endsWith('.ts')) return hashSourceTree(import.meta.dir);
    const sourceRoot = path.resolve(import.meta.dir, '..');
    const lockHash = hashBytes(readFileSync(path.join(sourceRoot, '..', 'bun.lock')));
    return `${lockHash}:${hashSourceTree(sourceRoot)}`;
  } catch {
    // Unreadable inputs disable reuse rather than sharing an unverifiable cache key.
    return randomUUID();
  }
}
