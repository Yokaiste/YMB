...
import { createErrorCollector, YmbError } from '../errors.ts';
...
export async function materializeBuild(
  ...
  for (const bucketMods of priorityBuckets) {
    ...
    const bucketScriptsByModId = Map.groupBy(
      bucketPlan.selectedScripts,
      (script) => script.mod.config.id,
    );
    const bucketReplaceFilesByModId = Map.groupBy(
  ...
  // Check the composed result even on cache hits and byte-preserving script output.
  // All commands pass this gate before preview or live files can be replaced.
  reportProgress('Validating NDF outputs');
  const failures = createErrorCollector();
  for (const file of outputMap.values()) {
    if (!isNdfPath(file.targetRelativePath)) continue;
    await failures.collect(async () => {
      try {
        if (typeof file.content !== 'string' && !isUtf8(file.content)) {
          throw new YmbError('ParserError', {
            absolutePath: file.targetRelativePath,
            code: 'ndf.invalid_encoding',
            reason: 'NDF output is not valid UTF-8.',
            suggestion:
              'Return NDF as text, or encode it with TextEncoder. Keep binary assets in their own file formats.',
          });
        }
        const content =
        ...
        const owner = file.contributors.length === 1 ? file.contributors[0] : undefined;
        const scripts =
          owner && file.sourceType === 'script'
            ? plan.selectedScripts.filter(
                (script) =>
                  script.mod.config.id === owner.modId && script.patch?.config.id === owner.patchId,
              )
            : [];
        throw new YmbError(error.category, {
          ...owner,
          ...error.context,
          sourcePath: scripts.length === 1 ? scripts[0]?.absolutePath : undefined,
          details: [
            ...(error.context.details ?? []),
            'Fix the source patch, replacement or generator, then build again.',
            ...(owner
              ? []
              : file.contributors.map(
                  (owner) =>
                    `Contributor: ${owner.modId}${owner.patchId ? ` / ${owner.patchId}` : ''}`,
                )),
          ],
        });
      }
