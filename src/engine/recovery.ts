...
export async function restoreOrDeleteTrackedTarget(args: {
  ...
  await rm(backupAbsolutePath, { force: true });
}
...
export async function runRecover(
  ...
  return withBuilderOperationLock(builderPath, 'recover', (context) =>
    runRecoverUnlocked(context, selection),
  );
}
...
async function runRecoverUnlocked(
  ...
  try {
    ...
    if (!selection.dryRun) {
      ...
      const sweep = await sweepOrphanedBackups(context, remainingEntriesByTarget);
  ...
  return toCommandOutput([...formatFindingGroups(findings), ...logs], {
    ...
    data: {
      counts: {
        ...
        deleted: deletedGeneratedCount,
        remaining: remainingEntriesByTarget.size,
        sweptBackups: sweptOrphanCount,
      },
      findings,
