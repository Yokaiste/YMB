...
export async function loadPreparedReplaceContent(
  plan: Pick<BuildPlan, 'context'>,
  ...
  if (targetIsNdf) {
    ...
    try {
      ...
      throw new YmbError(error.category, {
        ...
        modName: replaceFile.modName,
        patchId: replaceFile.patchId,
        sourcePath: replaceFile.sourceAbsolutePath,
      });
    }
  }
  return content;
...
export async function loadManifestEntryMap(
  plan: Pick<BuildPlan, 'context'>,
): Promise<Map<string, SyncManifestEntry>> {
