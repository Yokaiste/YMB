...
export async function collectCleanupTargets(
  ...
  const protectedPaths = [...targets.values()]
    .filter((target) => target.unsafeToRemove)
    .map((target) => toPathKey(target.absolutePath));
  for (const target of targets.values()) {
    const targetKey = toPathKey(target.absolutePath);
    target.unsafeToRemove ||= protectedPaths.some((protectedPath) =>
      pathsOverlap(targetKey, protectedPath),
    );
  }

  const reservedPaths = [plan.context.operationLockRoot, plan.context.stateTransactionRoot].map(
    toPathKey,
  );
  return [...targets.values()]
    .filter((target) => {
      const targetKey = toPathKey(target.absolutePath);
      return !reservedPaths.some((reservedPath) => pathsOverlap(targetKey, reservedPath));
    })
