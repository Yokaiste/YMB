...
export interface BuildScriptNdfTools {
  ...
  readNumericDefinitions(text: string): Record<string, number>;
  matcher(match: unknown): (block: BuildScriptNdfBlock) => boolean;
...
export interface BuildScriptTools {
  ...
  patch(text: string, target: { file: string; operations: readonly unknown[] }): Promise<string>;
