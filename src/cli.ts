...
export async function runCli(argv: string[]): Promise<void> {
  ...
  const program = createCliProgram(asJson);
  try {
    await program.parseAsync(argv);
  } catch (error) {
    if (error instanceof CommanderError && error.exitCode === 0) {
      if (asJson)
        printJson(buildJsonHelp(program, commandName, error.code === 'commander.version'));
      return;
    }
...
export function createCliProgram(suppressParseErrors = false): Command {
  ...
  const finishReportedCommand = <TOptions extends CommonOptions>(
    ...
    command.option('--verbose', 'Include routine file details as well as notices');
    addJsonOption(command);
    command.action(async (options: TOptions) => {
      options.json ||= program.opts<{ json?: boolean }>().json ?? false;
      if (!options.json) {
        renderCliTitleOnce();
      }
  ...
  initCommand.action(async (options: InitCommandOptions & { ymbPath?: string; json?: boolean }) => {
    ...
    try {
      const created = await runInit(options.ymbPath, { ...options, interactive: !options.json });
      if (options.json) {
...
function getCliTitleLines(): string[] {
  return [
    '',
    'Y   Y   M   M   BBBB',
    ' Y Y    MM MM   B   B',
...
/**
 * Result layout: headline, useful numbers and paths, issues, then the next step.
 * Labels share one padded column so the answer to
...
function formatCommandOutput(
  ...
  if (nextSteps.length > 0) outputLines.push('', 'Next', ...nextSteps.map((step) => `  ${step}`));
  return outputLines;
}
...
function describeSelection(commandName: ReportedCommandName, selection: SelectionInput): string {
  ...
  if (selection.dryRun) {
    parts.push('dry run, game and preview unchanged');
  }
...
function formatInitOutput(created: InitResult, durationMs: number): string[] {
  return [
    ...
    ...formatFactLines(
      [
        countFact('created', [['file', created.data?.files.length ?? created.lines.length]]),
    ...
    '',
    'Next',
    `  Run \`build --mod ${created.data?.id ?? '<id>'}\` to see what it produces.`,
  ];
}
