...
async function importRuntimeScript<T>(
  ...
  // Match Bun's worker paths so Windows 8.3 aliases cannot load a second ymb/api class.
  const canonicalPath = await realpath(absolutePath);
  const importUrl = pathToFileURL(canonicalPath);
  if (useCache) {
    importUrl.searchParams.set('ymb-runtime', hashText(`${canonicalPath}\n${scriptSource}`));
  } else {
    cacheBustCounter += 1;
