...
import { type BuildScriptCacheTools, ScriptToolError } from 'ymb/api';
import {
  getCacheSalt,
  pruneCacheDirectory,
  readCacheEntry,
    ...
    async writeJson(namespace: string, key: string, value: unknown): Promise<void> {
      ...
      try {
        ...
        await pruneCacheDirectory(path.dirname(cachePath), {
          maxEntries: plan.context.builderConfig.settings.scriptCacheMaxEntriesPerNamespace,
          maxBytes: plan.context.builderConfig.settings.scriptCacheMaxBytesPerNamespace,
        });
      } catch {
...
function assertCacheComponent(value: string, label: string): void {
  if (!CACHE_COMPONENT_PATTERN.test(value) || value === '.' || value === '..') {
    throw new ScriptToolError({
      reason: `Script cache ${label} \`${value}\` is invalid.`,
      suggestion: `Use only letters, numbers, dots, underscores, and hyphens in cache ${label}s, excluding \`.\` and \`..\`.`,
    });
  }
}
