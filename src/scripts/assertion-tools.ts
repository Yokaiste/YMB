import {
  type BuildScriptAssertionOptions,
  type BuildScriptAssertionTools,
  type BuildScriptSelfCheck,
  ScriptToolError,
} from 'ymb/api';
...
export const SCRIPT_ASSERTION_TOOLS: BuildScriptAssertionTools = Object.freeze({
  ok,
  ...
  async all(checks: BuildScriptSelfCheck[]): Promise<void> {
    ...
    if (failures.length === 1) {
      throw new ScriptToolError(firstFailure.options);
    }
