...
import type { BuildScriptTestContext, ScriptTestReport, ScriptTestResult } from '../api.ts';
...
export function assertScriptTestResultsPassed(
  ...
  if (failures.length === 1) {
    ...
    ensure(failure, 'ScriptError', {
      absolutePath: context.testAbsolutePath,
      reason: 'The failed script-test result could not be resolved.',
      suggestion: 'Re-run the script test and inspect its result payload.',
    });
