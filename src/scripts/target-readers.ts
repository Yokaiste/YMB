import { loadManifestEntryMap, loadPreparedReplaceContent } from '../engine/replace-materialize.ts';
...
interface TargetReaderState {
  ...
  manifestByTarget?: Promise<Map<string, SyncManifestEntry>>;
}
...
export async function readTargetText(
  ...
  const normalizedPath = assertGameRelativePath(relativePath, plan.context.modRoot);
  ...
  const replaceFile = state.replaceFilesByTarget.get(toPathKey(normalizedPath));
  if (replaceFile) {
    const prepared = await readReplacePreviewContent(plan, state, replaceFile);
    const content =
      typeof prepared === 'string' ? prepared : Buffer.from(prepared).toString('utf8');
    recordObservedRead(state, normalizedPath, 'text', hashText(content));
    return content;
  }
...
export async function readTargetBinary(
  ...
  if (replaceFile) {
    const prepared = await readReplacePreviewContent(plan, state, replaceFile);
    const content = typeof prepared === 'string' ? toBytes(prepared) : prepared;
    recordObservedRead(state, normalizedPath, 'binary', hashBytes(content));
    return content;
  }
...
function readReplacePreviewContent(
  plan: ScriptRuntimePlan,
  state: TargetReaderState,
  replaceFile: ReplaceFile,
): Promise<string | Uint8Array> {
  state.manifestByTarget ??= loadManifestEntryMap(plan);
  return state.manifestByTarget.then((manifest) =>
    loadPreparedReplaceContent(plan, replaceFile, manifest),
  );
}
