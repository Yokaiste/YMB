  ...
  if (error instanceof YmbError) {
    return {
      ...
      context: {
        ...fallbackContext,
        sourcePath:
          fallbackContext.absolutePath !== error.context.absolutePath
            ? fallbackContext.absolutePath
            : undefined,
        ...error.context,
      },
...
export async function runIpcWorker<
  TRequest extends object,
  TResponse extends { ok: boolean },
>(options: {
  childPath: string;
  cwd: string;
  request: TRequest;
  ...
  const { promise: responsePromise, resolve: resolveResponse } = Promise.withResolvers<TResponse>();
  ...
  const result = await awaitIpcChildResult(
    child,
    responsePromise,
    options.errorContext,
    options.timeoutSeconds,
  );
  if (result.response?.ok && result.exitCode !== 0) {
    throw new YmbError('ScriptError', {
      absolutePath: options.errorContext.absolutePath,
      modId: options.errorContext.modId,
      modName: options.errorContext.modName,
      patchId: options.errorContext.patchId,
      reason: `${options.errorContext.subjectLabel} returned a result but exited with code ${result.exitCode}.`,
      suggestion: 'Fix the worker error before using its output.',
      details: result.stderrText ? [result.stderrText] : [],
    });
  }
  return result;
}
