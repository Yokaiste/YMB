...
export async function ensureTextState(
  ...
  const existingState = states.get(targetKey);
  if (existingState) {
...
export async function resolveScriptBaseText(
  ...
  const targetKey = toPathKey(targetRelativePath);
  const cachedBaseText = baseTextCache.get(targetKey);
  if (cachedBaseText !== undefined) {
    return cachedBaseText;
  }
  const existingGenerated = existingGeneratedTextByTarget.get(targetKey);
  ...
  const baseText = await readTrackedTextCooperative(plan.context, absolutePath, yieldController);
