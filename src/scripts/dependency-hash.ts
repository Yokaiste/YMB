...
export async function collectScriptDependencySources(args: {
  ...
  while (pending.length > 0) {
    ...
    const sourceBytes = await Bun.file(currentPath).bytes();
    ...
    if (!/\.(?:[cm]?[jt]s|[jt]sx)$/i.test(currentPath)) continue;
    for (const specifier of collectRelativeSpecifiers(
...
async function resolveLocalScriptDependency(
  ...
  try {
    return Bun.resolveSync(fileSpecifier, importerDirectory);
  } catch {
    const assetPath = path.resolve(importerDirectory, fileSpecifier);
    return (await isFile(assetPath)) ? assetPath : undefined;
  }
}
