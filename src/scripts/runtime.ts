...
import type { GeneratedScriptFile } from '../api.ts';
...
export type ScriptRuntimeOutputPayload =
  | GeneratedScriptFile
  | {
      targetRelativePath: string;
      storedContent: { fileName: string; type: 'text' | 'binary' };
...
type ScriptRuntimeResponse =
  | {
      ok: true;
      outputs: ScriptRuntimeOutputPayload[];
    }
  | {
      ok: false;
...
async function runScriptInSubprocess(
  ...
  const exchangeRoot = await mkdtemp(path.join(tmpdir(), 'ymb-runtime-'));
  try {
    ...
    if (resolvedResponse.ok) {
      return await Promise.all(
        resolvedResponse.outputs.map((output) => loadScriptRuntimeOutput(output, exchangeRoot)),
      );
    }
...
const LARGE_SCRIPT_OUTPUT_BYTES = 256 * 1024;

export async function storeScriptRuntimeOutputs(
  outputs: GeneratedScriptFile[],
  exchangeRoot: string,
): Promise<ScriptRuntimeOutputPayload[]> {
  return await Promise.all(
    outputs.map(async (output, index) => {
      const byteLength =
        typeof output.content === 'string'
          ? Buffer.byteLength(output.content)
          : output.content.byteLength;
      if (byteLength < LARGE_SCRIPT_OUTPUT_BYTES) return output;

      const type = typeof output.content === 'string' ? 'text' : 'binary';
      const fileName = `${index}.${type === 'text' ? 'txt' : 'bin'}`;
      await Bun.write(path.join(exchangeRoot, fileName), output.content);
      return {
        targetRelativePath: output.targetRelativePath,
        storedContent: { fileName, type },
        ...(output.generatedBlockOwnerPaths
          ? { generatedBlockOwnerPaths: output.generatedBlockOwnerPaths }
          : {}),
      };
    }),
  );
...
async function loadScriptRuntimeOutput(
  output: ScriptRuntimeOutputPayload,
  exchangeRoot: string,
): Promise<GeneratedScriptFile> {
  if (!('storedContent' in output)) return output;
  const absolutePath = path.resolve(exchangeRoot, output.storedContent.fileName);
  const relativePath = path.relative(exchangeRoot, absolutePath);
  ensure(
    relativePath.length > 0 && !relativePath.startsWith('..') && !path.isAbsolute(relativePath),
    'ScriptError',
    {
      absolutePath,
      reason: 'Generation script worker returned an invalid output exchange path.',
      suggestion: 'Re-run the build. If it persists, inspect the script runtime worker.',
    },
  );
  const file = Bun.file(absolutePath);
  ensure(await file.exists(), 'ScriptError', {
    absolutePath,
    reason: 'Generation script worker output was missing from the exchange directory.',
    suggestion: 'Re-run the build. If it persists, inspect available temporary disk space.',
  });
  return {
    targetRelativePath: output.targetRelativePath,
    content:
      output.storedContent.type === 'text'
        ? await file.text()
