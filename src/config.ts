...
const operationSchema: z.ZodType<NdfOperation> = z
  ...
  .superRefine((value, ctx) => {
    if (value.op === 'copy' && !value.destination) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: '`copy` operations require `destination`.',
        path: ['destination'],
      });
    }

    if (value.op === 'modify' && value.value === undefined && value.changes === undefined) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: '`modify` operations require either `value` or `changes`.',
      });
    }
  });

const scriptSchema: z.ZodType<ScriptConfig> = z.object({
  path: z.string().min(1),
