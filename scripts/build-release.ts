...
if (!Bun.semver.satisfies(Bun.version, packageDefinition.engines.runtime.version)) {
  throw new Error(
    `Build runtime ${Bun.version} does not satisfy ${packageDefinition.engines.runtime.version}.`,
  );
}
...
await Promise.all([
  ...
  // Releases start clean; the checkout config may contain personal overrides and paths.
  writeFile(
    path.join(metadata.releaseRoot, BUILDER_CONFIG.builderConfigFileName),
    `version: ${createDefaultBuilderProjectConfig().version}\n\n# Local choices: docs/configuration.md#customizing-installed-mods.\n`,
    'utf8',
  ),
...
await cp(metadata.releaseRoot, metadata.system.root, {
  recursive: true,
  filter: (source) => source !== metadata.runtimeRoot,
});
...
await Promise.all([
  ...
  writeFile(metadata.notesPath, renderReleaseNotes(metadata), 'utf8'),
]);
