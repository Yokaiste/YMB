...
async function expectPlanError(
  builderPath: string,
  expectedCategory: YmbError['category'],
): Promise<YmbError> {
  try {
    ...
    expect(ymbError.category).toBe(expectedCategory);
    return ymbError;
  }
...
`,
    ...
    await expectPlanError(workspace.builderPath, 'LayoutError');
  });
...
`),
    ...
    await expectPlanError(fileAncestor.builderPath, 'LayoutError');
...
`),
    ...
    await expectPlanError(plannedHierarchy.builderPath, 'LayoutError');
...
`),
    ...
    await expectPlanError(replaceCollision.builderPath, 'ConflictError');
...
`,
    ...
    await expectPlanError(deletionCollision.builderPath, 'ConflictError');
...
`,
    ...
    await expectPlanError(hierarchicalCollision.builderPath, 'ConflictError');
  });
});
...
`),
    ...
    await expectPlanError(workspace.builderPath, 'IoError');
  });
});
