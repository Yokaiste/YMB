...
`;
  ...
  test('copies and modifies action-DSL objects with local is declarations', () => {
...
`;
      ...
      operations: [
        {
          ...
          destination: { kind: 'sibling', name: 'fx_impact_sol_YN_Tactical_Nuke' },
        },
        {
          op: 'modify',
          selector: {
    ...
    const output = applyPatchTarget(source, target, application, 'C:/fixture/FX_Impact_Nuke.ndf');
...
`;
  ...
  test('collection type selector accepts alternative spellings', () => {
...
`;
  ...
  test('collection type selector picks the Nth entry of that type', () => {
    const source = `export WeaponDescriptor_Swarm is TWeaponManagerModuleDescriptor
(
...
`;
    ...
    const output = applyPatchTarget(source, target, application, 'C:/fixture/WeaponDescriptor.ndf');
  ...
  test('collection type selector still refuses an ambiguous match', () => {
    const source = `export WeaponDescriptor_Swarm is TWeaponManagerModuleDescriptor
(
    MountedWeaponDescriptorList =
    [
...
`;
    ...
    expect(() =>
      applyPatchTarget(source, target, application, 'C:/fixture/WeaponDescriptor.ndf'),
    ).toThrow(/matched multiple entries/);
  });

  test('adds to an unnamed top-level block by semantic type', () => {
...
`;
      ...
      operations: [
        {
          ...
          value: {
            $raw: '("YSM", TUISpecificCountryInfos(Country = "YSM")),',
          },
        },
      ],
    };
    ...
    expect(output).toContain('("YSM", TUISpecificCountryInfos(Country = "YSM"))');
...
`;
  ...
  test('matches collection entries by a nested selector field without positional paths', () => {
