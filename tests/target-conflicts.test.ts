...
import type { DiscoveredMod, ReplaceFile } from '../src/types.ts';

import { createTestBuilderContext } from './helpers/planner.ts';
...
const context = createTestBuilderContext();

test('a later layering mod cannot write the same replacement twice', async () => {
  const base = createMod('base');
  const addon = createMod('addon', { priority: 1, allowWriteToModifiedFiles: true });
  await expect(
    detectTargetConflictsCooperative(
      context,
      [base, addon],
      [],
      [createReplaceFile(base), createReplaceFile(addon), createReplaceFile(addon)],
      [],
    ),
  ).rejects.toThrow();
});
