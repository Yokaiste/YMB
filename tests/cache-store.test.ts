...
describe('cache salt', () => {
  test('follows source and bundled code while ignoring game files and docs', async () => {
    ...
    try {
      ...
      const sourceDigest = hashSourceTree(root);
      await writeFile(path.join(root, 'runtime.js'), 'export const value = 1;', 'utf8');
      expect(hashSourceTree(root)).not.toBe(sourceDigest);
    } finally {
  ...
  test('salts cache keys with the package version and the builder sources', () => {
    ...
    expect(salt).toContain(`:${Bun.version}:`);
