...
describe('collecting failures instead of stopping at the first', () => {
  ...
  test('keeps every error location instead of dropping entries after a print limit', () => {
    const failures = createErrorCollector();
    ...
    expect(group.errors.length).toBe(40);
    expect(group.omittedCount).toBe(0);
    expect(group.message).toContain('40 problems found');
    expect(group.message).toContain('file-39');
    expect(group.message).not.toContain('more not shown');
  });
...
describe('reading a report of several failures', () => {
  test('one repeated error message and fix list every affected file', () => {
    const errors = Array.from(
      { length: 40 },
      (_, index) =>
        new YmbError('IoError', {
          absolutePath: `C:/mod/GameData/file-${index}.ndf`,
          reason: 'Target file is missing.',
          suggestion: 'Fix the target path.',
        }),
    );
    const report = formatErrorGroupLines(errors).join('\n');
    expect(report.split('Target file is missing.')).toHaveLength(2);
    expect(report.split('Fix the target path.')).toHaveLength(2);
    expect(report).toContain('(40 entries)');
    for (const error of errors) {
      expect(report.split(error.context.absolutePath)).toHaveLength(2);
    }
  });
