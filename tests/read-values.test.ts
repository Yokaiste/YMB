...
describe('reading values out of the game', () => {
  test('numeric definitions supply current enum ordinals', async () => {
...
`),
    ...
    await writeFile(
      path.join(builderPath, '..', CONSTANTS_FILE),
      'Category_First is 41\nCategory_Second is 410\n',
    );
    expect((await planVariables(builderPath)).conversionFactor).toBe(41);
    await writeFile(path.join(builderPath, '..', CONSTANTS_FILE), 'Category_First is 72\n');
    expect((await planVariables(builderPath)).conversionFactor).toBe(72);
  });
...
`),
    ...
    await expect(planVariables(builderPath)).rejects.toThrow();
  });
