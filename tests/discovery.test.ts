...
`,
      ...
      const loadAttempt = loadPatchConfig(patchConfigPath);
      await expect(loadAttempt).rejects.toThrow('patch config fields are invalid.');
      await expect(loadAttempt).rejects.toThrow('newer than this YMB build supports');
    } finally {
      await rm(tempRoot, { recursive: true, force: true });
    }
  });
...
`,
      ...
      await expect(loadAttempt).rejects.toThrow('patch config fields are invalid.');
      await expect(loadAttempt).rejects.toThrow(
        'targets.0.operations.0.leadingComment: Field-path `modify` operations do not support `leadingComment`.',
      );
      await expect(loadAttempt).rejects.toThrow(
        'targets.0.operations.1.selector: Object `add` operations support `by: name` and `by: index` only.',
      );
      await expect(loadAttempt).rejects.toThrow(
        'targets.0.operations.2.selector: `remove` operations support `object` selectors using `by: name`, `by: index`, or `by: match`, and `field` selectors using `by: path` only.',
      );
    } finally {
      await rm(tempRoot, { recursive: true, force: true });
    }
