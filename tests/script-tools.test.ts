...
(
  ...
  test('reads direct and deep field values', () => {
    ...
    expect(ndf.readFields(block, ['FrontArmor', 'Inner', 'Plain'])).toEqual({
      FrontArmor: '5',
      Plain: '1',
    });
    expect(ndf.readFieldsDeep(block, ['FrontArmor', 'Inner', 'Missing'])).toEqual({
      FrontArmor: '5',
      Inner: '9',
    });
  });
  ...
  test('reads the trailing comment of a field, ignoring quoted slashes', () => {
    const commented = 'Descriptor is TFoo\n(\n    FrontArmor = 5 // skip-me\n    Path = "a//b"\n)';
    expect(ndf.findFieldWithComment(commented, 'FrontArmor')?.trailingComment).toBe('skip-me');
  ...
  test('reads paths with dotted selectors using the same segments as NDF patches', () => {
    const text = `TContainer (
      Items = [ TItem ( Identity = TIdentity ( Name = 'sample' ) Value = 7 ) ]
    )`;
    const segments = ['Items', "[Identity.Name='sample']", 'Value'];
    expect(ndf.readPath(text, segments)).toBe('7');
    expect(ndf.readPath(text, segments.join('.'))).toBe('7');
    expect(ndf.readPath(text, "Items.[Identity.Name='missing'].Value")).toBeUndefined();
  });
});
...
describe('script ndf tools: generated blocks', () => {
  test('lists and strips generated blocks produced by the builder renderer', () => {
    const rendered = renderGeneratedBlock({
      ownerId: 'sample_mod | decks',
      blocks: ['Descriptor_Deck is TDeck\n(\n)'],
      sourcePath: 'mods/sample_mod/config/patch/decks',
    });
    const content = `Head is TThing\n(\n)\n\n${rendered}`;

    const blocks = ndf.listGeneratedBlocks(content);
    expect(blocks).toHaveLength(1);
    expect(blocks[0]?.id).toBe('sample_mod | decks');
    expect(blocks[0]?.sourcePath).toBe('mods/sample_mod/config/patch/decks');
    ...
    expect(ndf.generatedBlockMarkers('sample_mod | decks')).toEqual({
      start: '// YMB GENERATED BLOCK START | sample_mod | decks',
      end: '// YMB GENERATED BLOCK END | sample_mod | decks',
    });
  });
});
...
`;
  ...
  test.each(['Charlie', 'https://example.invalid/sample'])(
    'preserves owned-block markers after a collection entry separator: %s',
    (value) => {
      const ownerId = 'sample.feature';
      const block = ndf.renderGeneratedBlock({
        ownerId,
        blocks: [`(3, ${JSON.stringify(value)}),`],
      });
      const updated = ndf.insertIntoCollection(source, 'DivisionRules.DivisionIds', block);
      const owned = ndf.listGeneratedBlocks(updated);
      expect(owned).toHaveLength(1);
      expect(owned[0]?.id).toBe(ownerId);
      expect(owned[0]?.fullText.trimEnd().endsWith(ndf.generatedBlockMarkers(ownerId).end)).toBe(
        true,
      );
      expect(ndf.insertIntoCollection(updated, 'DivisionRules.DivisionIds', block)).toBe(updated);
      const parent = ndf.findNamedBlock(updated, 'DivisionRules');
      if (!parent) throw new Error('Missing DivisionRules block');
      const collection = ndf.readField(parent.text, 'DivisionIds');
      if (collection === undefined) throw new Error('Missing DivisionIds collection');
      const entries = ndf.findCollectionEntries(collection);
  ...
  test('rejects a path without a field segment', () => {
    expect(() =>
      ndf.insertIntoCollection(source, 'DivisionRules', { $raw: '(9, "X"),' }),
    ).toThrow();
  });
});
