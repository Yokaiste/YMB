...
import { NdfTextBuffer } from '../src/patch/ndf/buffer.ts';
...
describe('the text buffer a target is applied to', () => {
  ...
  test('a run of edits produces exactly what splicing the string would have', () => {
    const buffer = new NdfTextBuffer(source);
    let expected = source;
    ...
    expect(buffer.includes(expected)).toBe(true);
    expect(buffer.includes(`${expected}missing`)).toBe(false);
  ...
  test('indentation is read without joining the file', () => {
    const buffer = new NdfTextBuffer(`Outer\n(\n        Inner = 1\n)\n`);
