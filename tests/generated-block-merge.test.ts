...
describe('generated block merge', () => {
  test('applies when only the current script block changes', () => {
    ...
    const next = current.replace('owner-old', 'owner-new');
  ...
  test('rejects duplicate generated block ids instead of silently comparing only the last block', () => {
    const current = [
      '// YMB GENERATED BLOCK START | patches/core.ts',
      'first-old',
      '// YMB GENERATED BLOCK END | patches/core.ts',
    ...
    const next = current.replace('first-old', 'first-new');

    const result = tryMergeGeneratedBlocks(current, next, 'patches/core.ts');
    expect(result.kind).toBe('conflict');
    if (result.kind !== 'conflict') {
      return;
    }
    expect(result.details).toEqual(['Duplicate generated block: patches/core.ts']);
  });
});
