...
const ESCAPING_PATHS = [
  ...
  ['windows drive-relative path', 'C:outside.ndf'],
...
describe('owned relative path guard', () => {
  test.each(['file.txt:stream', 'NUL.txt', 'folder./file', 'trailing /file', 'bad\u0000.txt'])(
    'rejects Windows-unsafe owned path %s',
    (candidate) => {
...
describe('target path keys', () => {
  test('treat WARNO paths as case-insensitive and separator-insensitive', () => {
    ...
    expect(toPathKey('GameData\\Generated\\Gameplay\\Units.ndf')).toBe(key);
    expect(toPathKey('GameData/./Generated/Gameplay/Unused/../Units.ndf')).toBe(key);
  });
