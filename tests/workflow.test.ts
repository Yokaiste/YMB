...
`,
  ...
  test.each(['text', 'bytes', 'invalid-utf8'])(
    'all commands reject malformed %s NDF output before replacing files',
    async (encoding) => {
      const builderPath = await createTempBuilder();
      const target = 'GameData/Generated/Example.NDF';
      const valid = 'Root is T(Children = [Child()])\n';
      const invalid = 'Root is T(Children = [] Child(),)\n';
      const script = (content: string) => `export default function generate() {
      const content = ${JSON.stringify(content)};
      return {targetRelativePath: ${JSON.stringify(target)}, content: ${encoding === 'invalid-utf8' && content === invalid ? 'new Uint8Array([34, 255, 34])' : encoding !== 'text' ? 'new TextEncoder().encode(content)' : 'content'}};
    }`;
      const files = (content: string) => ({
        'config/ymb.mod.yaml':
          'version: 1\nid: validation_pack\nname: Sample\nscripts:\n  - path: generate.ts\n',
        'config/generate.ts': script(content),
      });
      await writeModFixture(builderPath, 'validation-pack', files(valid));
      const selection = createSelection({ modFilters: ['validation_pack'], yes: true });
      const livePath = path.join(path.dirname(builderPath), ...target.split('/'));
      await Bun.write(livePath, valid);
      await runBuild(builderPath, selection);
      const previewPath = buildOutputPath(builderPath, ...target.split('/'));
      const preview = await Bun.file(previewPath).text();
      await writeModFixture(builderPath, 'validation-pack', files(invalid));
      for (const command of [runValidate, runBuild, runSync]) {
        const error = await command(builderPath, selection).catch((error) => error);
        expect(error).toBeInstanceOf(YmbError);
        expect((error as YmbError).context).toMatchObject({
          code: encoding === 'invalid-utf8' ? 'ndf.invalid_encoding' : 'ndf.expected_assignment',
          modId: 'validation_pack',
          absolutePath: target,
          sourcePath: path.join(builderPath, 'mods/validation-pack/config/generate.ts'),
        });
      }
      await expect(runBuild(builderPath, { ...selection, dryRun: true })).rejects.toBeInstanceOf(
        YmbError,
      );
      expect(await Bun.file(livePath).text()).toBe(valid);
      expect(await Bun.file(previewPath).text()).toBe(preview);
    },
  );
...
scripts: []
`,
      [`config/replace/CommonData/Text/${modRootName}-replaced.ndf`]:
        '// Priority replace content\n',
    });
