...
`,
  ...
  test('validate, build, and sync run configured script tests', async () => {
    ...
    expect(
      syncLines.some((line) =>
        /script test (?:cached )?ok -> generate-mod-summary\.test\.ts :: mod-level summary script returns the expected output/.test(
          line,
        ),
      ),
    ).toBe(true);
    expect(
      syncLines.some((line) =>
        /script test (?:cached )?ok -> generate-armor-summary\.test\.ts :: patch-level armor summary script returns the expected output/.test(
          line,
        ),
      ),
    ).toBe(true);
...
`,
    ...
    expect(secondBuild.summary?.join('\n')).toContain('merged: 1 hit');
  });
...
`,
    ...
    const selection = {
      scope: 'prod' as const,
      modFilters: ['base_replace_pack', 'branding_pack'],
      patchFilters: [],
      dryRun: false,
      verbose: false,
      yes: false,
    ...
    expect(
      await Bun.file(
        path.join(builderPath, '.ymb-build', 'output', 'CommonData', 'Text', 'layered.txt'),
      ).text(),
    ).toBe('Layered title\nPreserved line\n');
    ...
    const baseConfig = await Bun.file(baseConfigPath).text();
    await Bun.write(baseConfigPath, baseConfig.replace('true', 'false'));
    await expect(runBuild(builderPath, selection)).rejects.toThrow(
      'Script output collides with a replace target',
    );
  });
});
