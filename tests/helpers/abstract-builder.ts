...
interface AbstractBuilderWorkspaceOptions {
  builderDirectoryName?: string;
  builderConfig?: string;
}

export async function createAbstractBuilderWorkspace(
  tempRoots: string[],
  options: AbstractBuilderWorkspaceOptions = {},
): Promise<AbstractBuilderWorkspace> {
  const rootPath = await mkdtemp(path.join(tmpdir(), 'ymb-test-'));
  tempRoots.push(rootPath);
  const builderDirectoryName = options.builderDirectoryName ?? 'YMB';
  const builderPath = path.join(rootPath, builderDirectoryName);
...
`,
    ...
    [`${builderDirectoryName}/mods/sample-pack/config/ymb.mod.yaml`]: `version: 1
id: sample_pack
name: Sample Pack
variables:
...
tempPaths:
  ...
  - path: .ymb-mod-important.json
    unsafeToRemove: true
`,
    [`${builderDirectoryName}/mods/sample-pack/config/generate-mod-summary.ts`]: `export default async function generateModSummary(context: {
...
(
    ...
    Text = \${JSON.stringify(replaced.trim())}
)
\`,
  };
}
`,
    [`${builderDirectoryName}/mods/sample-pack/config/generate-mod-summary.test.ts`]: `import generateModSummary from './generate-mod-summary.ts';
...
`,
    [`${builderDirectoryName}/mods/sample-pack/config/patch/armor/ymb.patch.yaml`]: `version: 1
id: balance.armor
name: Armor Tweaks
enabled: true
scope: prod
    ...
    operations:
      ...
      - op: modify
        ...
        changes:
          ...
          FrontArmor: 9
`,
    [`${builderDirectoryName}/mods/sample-pack/config/patch/armor/generate-armor-summary.ts`]: `export default async function generateArmorSummary(context: {
  variables: Record<string, unknown>;
  readTarget(relativePath: string): Promise<string>;
}): Promise<{ targetRelativePath: string; content: string }> {
...
`,
    [`${builderDirectoryName}/mods/sample-pack/config/patch/armor/generate-armor-summary.test.ts`]: `import generateArmorSummary from './generate-armor-summary.ts';
...
`,
    [`${builderDirectoryName}/mods/sample-pack/config/replace/CommonData/Text/\${modRootName}-replaced.ndf`]:
      '// Replaced content for ${modName}\n',
  };
  if (options.builderConfig) {
    files[`${builderDirectoryName}/ymb.config.yaml`] = options.builderConfig;
  }
...
export function syntheticBuilderPath(builderPath: string, ...segments: string[]): string {
  return path.join(builderPath, 'tests', 'synthetic', path.basename(builderPath), ...segments);
}

export function sampleModConfigPath(builderPath: string, ...segments: string[]): string {
  return path.join(builderPath, 'mods', 'sample-pack', 'config', ...segments);
}
...
export async function writeModFixture(
  ...
  const prefixedFiles = Object.fromEntries(
    Object.entries(files).map(([relativePath, content]) => [
      path
        .join(path.basename(builderPath), 'mods', modDirectoryName, relativePath)
        .replaceAll('\\', '/'),
      content,
    ]),
  );
...
/**
 * One summary fact by name. Tests read the value a command reported, not the
 * text a renderer happened to lay it out as.
 */
export function summaryValue(lines: CommandOutputLines, label: string): string {
  return lines.summary?.find((fact) => fact.label === label)?.value ?? '';
}

/** The whole summary as one string, for a test that only needs to find a phrase. */
export function summaryText(lines: CommandOutputLines): string {
  return (lines.summary ?? []).map((fact) => `${fact.label}: ${fact.value}`).join('\n');
}
