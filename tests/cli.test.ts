...
import { createProgressDisplay, runCli } from '../src/cli.ts';
...
describe('cli safety', () => {
  test('help output renders the CLI title for root and subcommand help', () => {
    ...
    for (const output of [rootHelpOutput, buildHelpOutput]) {
      expect(output).toContain('--help');
      expect(output.length).toBeGreaterThan(0);
    }
    expect(rootHelpOutput).toContain('doctor');
    expect(buildHelpOutput).toContain('--mod');
    expect(rootHelpTitle.trim().split(/\r?\n/).length).toBeGreaterThan(2);
    expect(buildHelpTitle).toBe(rootHelpTitle);
    expect(
      [...(rootHelpOutput + rootHelpTitle + buildHelpOutput + buildHelpTitle)].every(
  ...
  test('JSON mode keeps command-line parser failures machine-readable', () => {
    ...
    for (const testCase of cases) {
      ...
      expect(payload.errors).toEqual([
        expect.objectContaining({ category: 'CommandError', reason: expect.any(String) }),
      ]);
    }
  });
  ...
  test('sync requires --yes before touching the live mod root', async () => {
    ...
    const livePath = path.join(path.dirname(builderPath), 'GameData/Generated/Gameplay/Units.ndf');
    const before = await Bun.file(livePath).text();
    ...
    expect(await Bun.file(livePath).text()).toBe(before);
  });
  ...
  test('cleanup --all requires --yes before removing recovery data', async () => {
    const builderPath = await createTempBuilder();
    const recoveryFile = path.join(builderPath, '.ymb-state/keep.bin');
    const recoveryBytes = new Uint8Array([0, 17, 255, 3]);
    await Bun.write(recoveryFile, recoveryBytes);
    const errorLines: string[] = [];
    console.log = () => {};
    console.error = (...args: unknown[]) => {
      errorLines.push(args.map(String).join(' '));
    ...
    expect(process.exitCode).toBe(1);
    expect(errorLines.join('\n').length).toBeGreaterThan(0);
    expect(new Uint8Array(await Bun.file(recoveryFile).arrayBuffer())).toEqual(recoveryBytes);
  });

  test('sync runs with structured output after explicit confirmation', async () => {
    const builderPath = await createTempBuilder();
    const child = Bun.spawn(
      ['bun', 'run', './index.ts', 'sync', '--ymb-path', builderPath, '--yes', '--json'],
      { stdout: 'pipe', stderr: 'pipe' },
    );
    const payload = await new Response(child.stdout).json();
    expect(await child.exited).toBe(0);
    expect(payload).toMatchObject({ command: 'sync', ok: true });
  ...
  test('build supports --no-cache and reports cache bypass in the summary', async () => {
    ...
    expect(payload).toMatchObject({
      command: 'build',
      ok: true,
      selection: { useCache: false, dryRun: true },
    });
  });
  ...
  test('progress display redraws immediately when the status message changes', () => {
    ...
    try {
      ...
      expect(writes.join('')).toContain('.../sample-pack/config/generate-mod-summary.test.ts');
      ...
      expect(renderedOutput).toContain('.../sample-pack/config/generate-mod-summary.ts');
  ...
  test('sync progress remains monotonic while loading its manifest', () => {
    ...
    expect(loading.groupId).not.toBe(syncing.groupId);
...
`,
    ...
    expect(process.exitCode).toBe(0);
    const modConfig = Bun.YAML.parse(
      await Bun.file(path.join(builderPath, 'mods', 'new_pack', 'config', 'ymb.mod.yaml')).text(),
    );
    expect(modConfig).toMatchObject({
      id: 'new_pack',
      name: 'New Pack',
...
`,
    ...
    expect(listed.slice(0, shown).join('\n')).toContain('sample.absent');
    expect(listed[shown ?? 0]).toContain('--verbose');
