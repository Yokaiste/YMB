...
describe('script dependency hash helpers', () => {
  test('uses runtime resolution for JavaScript-to-TypeScript and JSON imports and hashes binary assets', async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), 'ymb-script-resolution-'));
    try {
      await Bun.write(
      ...
      await Bun.write(path.join(tempRoot, 'helper.ts'), 'export const value = 1;');
      await Bun.write(path.join(tempRoot, 'settings.json'), '{"value": 1}');
      await Bun.write(path.join(tempRoot, 'asset.bin'), new Uint8Array([0xff]));
      const collect = () =>
        collectScriptDependencySources({
          entryAbsolutePaths: [path.join(tempRoot, 'entry.ts')],
          rootAbsolutePath: tempRoot,
        });
      const before = await collect();
      expect(before.map((source) => source.relativePath)).toEqual([
        'asset.bin',
        'entry.ts',
        'helper.ts',
        'settings.json',
      ]);
      await Bun.write(path.join(tempRoot, 'asset.bin'), new Uint8Array([0xfe]));
