...
describe('script runtime output', () => {
  ...
  test('public contexts expose isolated authored-file and target I/O', async () => {
    ...
    try {
      ...
      const outsideRoot = path.join(tempRoot, 'outside-owned-root');
      const linkedRoot = path.join(script.patch?.absolutePath ?? '', 'linked-outside');
      await mkdir(outsideRoot, { recursive: true });
      await Bun.write(path.join(outsideRoot, 'escape.txt'), 'outside');
      await symlink(outsideRoot, linkedRoot, 'junction');
      await expect(
        publicContext.readOwnedTextIfExists('linked-outside/escape.txt'),
      ).rejects.toThrow();
  ...
  test.each([true, false])(
    'test file writes share both path scopes (patch: %s)',
    async (hasPatch) => {
      const tempRoot = await mkdtemp(path.join(tmpdir(), 'ymb-runtime-shared-io-'));
      const { context, script } = createTempScriptFixture(tempRoot);
      if (!hasPatch) delete script.patch;
      const plan = createScriptRuntimePlan(context);
      ...
      const modPath = path.relative(
        ...
        path.join(ownerRoot, 'state.txt'),
      );
      const diskFile = Bun.file(path.join(ownerRoot, 'state.txt'));

      try {
        await mkdir(ownerRoot, { recursive: true });
        await Bun.write(diskFile, 'original');
        const execution = createScriptTestExecutionContext(
          plan,
          script,
          new Map(),
          'generate.test.ts',
        );
        const io = execution.context;
        expect(await io.writeOwnedTextIfChanged('state.txt', 'first')).toBe(true);
        expect(await io.readModTextIfExists(modPath)).toBe('first');
        expect(await io.writeModTextIfChanged(modPath, 'first')).toBe(false);
        expect(await io.writeModTextIfChanged(modPath, '')).toBe(true);
        expect(await io.readOwnedTextIfExists('state.txt')).toBe('');
        expect(await io.writeOwnedTextIfChanged('state.txt', '')).toBe(false);
        expect(await diskFile.text()).toBe('original');
        expect(execution.getObservedScriptFileReads()).toEqual([
          {
            scope: 'owner',
        ...
        const next = createScriptTestExecutionContext(plan, script, new Map(), 'other.test.ts');
        expect(await next.context.readModTextIfExists(modPath)).toBe('original');
      } finally {
        await rm(tempRoot, { recursive: true, force: true });
      }
    },
...
describe('script runtime subprocess', () => {
  test('rejects a worker that returns output but fails during shutdown', async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), 'ymb-runtime-exit-'));
    const { context, script } = createTempScriptFixture(tempRoot);
    try {
      await Bun.write(
        script.absolutePath,
        [
          ...
          "  process.once('disconnect', () => { process.exitCode = 2; });",
          '  return [];',
          '}',
        ].join('\n'),
      );
      await expect(
        runScript(createScriptRuntimePlan(context), script, new Map()),
      ).rejects.toThrow();
    } finally {
      await rm(tempRoot, { recursive: true, force: true });
    }
