...
describe('builder config loading', () => {
  ...
  test('refuses a builder config path that is a directory', async () => {
    ...
    try {
      ...
      expect(failure.context.absolutePath).toBe(configPath);
  ...
  test('names the builder settings that do not fit the schema', async () => {
    ...
    try {
      ...
      const failure = await loadFailure(loadBuilderProjectConfig(configPath));
      expect(failure.category).toBe('ConfigError');
      expect(failure.context.absolutePath).toBe(configPath);
...
describe('source mod config loading', () => {
  test('rejects cyclic YAML aliases but accepts shared values', async () => {
    const root = await createConfigRoot();
    const configPath = path.join(root, 'ymb.mod.yaml');
    const prefix = 'version: 1\nid: sample_mod\nname: Sample\nvariables:\n';
    try {
      await writeFile(configPath, `${prefix}  loop: &loop\n    self: *loop\n`);
      await expect(loadModConfig(configPath)).rejects.toThrow();
      await writeFile(configPath, `${prefix}  first: &value [1, 2]\n  second: *value\n`);
      expect((await loadModConfig(configPath)).variables).toEqual({
        first: [1, 2],
        second: [1, 2],
      });
    } finally {
