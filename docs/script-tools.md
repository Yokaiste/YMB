...
Scripts can reuse declarative operations with
`await context.tools.patch(text, { file: 'GameData/Sample.ndf', operations: [...] })`.
It validates the same operation shapes, expands templates and `forEach` with the script's
variables, validates the NDF, and returns the patched text. It does not read or write the
target: return that text as a generated file to include it in the build.

`ndf.findObjects(text)` returns top-level and nested typed objects in source order, with
absolute character spans and names where available. `ndf.matcher(match)` compiles the same
conditions as `bulk` into a predicate for those blocks. For example,
`ndf.findObjects(text).filter(ndf.matcher({ conditions: [{ on: 'type', is: 'equals', value: 'TAmmo' }] }))`.
Comments and quoted object-like text are excluded from the object walk.

`ndf.readNumericDefinitions(text)` returns numeric exports such as `{ Category_First: 41 }`
from `Category_First is 41`. Each definition must occupy its own line, optionally with a
trailing comment. Nested values and expressions are not numeric definitions.
...
Readers use earlier generated output first, then the preview of a visible replacement or
file operation, then game data. Text and binary readers see the same prepared content,
including resolved templates. Reading another mod's output requires
[`allowWriteToModifiedFiles`](advanced.md#layering-one-mod-over-another).
...
Companion tests keep these writes in memory for that test. Both path scopes see changes
to the same file, while authored files on disk remain unchanged.

Paths must stay inside their owning folder and use valid Windows filenames. Drive-relative
paths, device names, alternate data streams, and trailing dots or spaces are rejected.
...
`readPath(text, path)` accepts field segments as an array or a dotted path using the
same collection selectors as patches, such as `Items.[Identity.Name='sample'].Value`.
...
**Check early while developing a generator:**

```ts
context.tools.ndf.assertValid(output, 'MyPack/Index.ndf');
```

YMB also validates the final combined output automatically. `ndf.validate()` returns
the same diagnostic without throwing: `ok: false` with `error.code`, `line`, `column`,
`offset`, and a suggested fix. A standalone value is accepted by these tools; several
top-level definitions need `Name is Value` declarations. Syntax validation does not
resolve the game's internal types or resources.
...
Namespace and key names allow letters, numbers, dots, underscores, and hyphens. Neither
may be `.` or `..`.
