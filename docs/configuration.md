...
Supported fields are capabilities, not a checklist to fill. When authoring a mod,
expose meaningful choices and keep defaults with their owner. See
[deciding what belongs in configuration](development.md#make-configuration-intentional).
...
## Customizing installed mods

Keep local choices in `YMB/ymb.config.yaml` so you do not have to edit each downloaded
patch. Source configs still define the defaults. The builder file can override variables
for every mod, for one mod, or for one patch:
...
Replace these example names with ids and variables from your mod. Ids must match exactly;
unknown mod or patch ids fail with a config error. Variable names belong to the source
mod: adding a name that no patch or script uses has no effect.

The last value wins in this order:

1. Source mod variables.
2. Source patch variables, when building that patch.
3. Builder-wide `variables`.
4. `mods.<id>.variables`.
5. `mods.<id>.patches.<patch-id>.variables`.

Objects merge recursively. For example, overriding `tuning.speed` keeps the existing
`tuning.vision`. Lists replace the whole list; `[]` clears it. Scalars and `null`
replace the previous value. Expressions resolve after merging, so derived values follow
your choices. Built-in identity variables such as `modId` remain reserved.

Nested values may reference siblings in the same object. For example,
`tuning: { base: 4, boosted: '${tuning.base * 2}' }` resolves `boosted` to 8.
Overriding `tuning.base` also updates `boosted`. Dot/bracket access, array entries,
aliases, and expression helpers use the same member resolution; real circular
references are rejected. A whole-object reference returns a fully resolved copy.

A disabled patch's dependencies are not disabled automatically. If an enabled feature
requires it, YMB reports that dependency; disable the dependent feature too or keep its
prerequisite enabled. Patch-only variables do not affect mod-wide scripts or replace files.

Save, run `build --mod <id>`, and inspect the preview. Changed settings invalidate the
relevant build/test cache. To undo a customization, remove its override and rebuild.
Back up `ymb.config.yaml` before manually extracting a new YMB release over it. Mod
installers that support these overrides preserve this file during updates.
...
instead, like any other selector that stopped matching. The check reads the finished project, including references from other files.
...
Patch variables override mod variables; nested objects keep unspecified fields. Local builder overrides take precedence over both. Built-ins like `${modId}` are
always available.
...
readValues:
  ...
  categoryNumber:
    file: GameData/Generated/Gameplay/Gfx/Enums/WeaponsMinMaxCategory.ndf
    path: MinMax_AutocanonHE
```
...
| `path` | A numeric definition name, or the same field path used by a `modify` selector. |

Notes:

- `path` can name a numeric definition such as `MinMax_AutocanonHE is 1`, or a top-level
  block followed by a field path. Use `@type:TSomeType` when the block has no name.
...
- A missing file, definition or field stops the build and says which
...
Safe cleanup also preserves that path's parent directories and contents, even if another
temporary-path entry labels them disposable.
...
Every install ships one at the YMB root. Only `version: 1` is active by default.
Uncomment a group and the values you need to customize mods, move a folder or tune a limit:
