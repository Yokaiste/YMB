...
| Field                       | Meaning                                       |
| --------------------------- | --------------------------------------------- |
| `version`                   | Required config format version, currently `1` |
...
Both config schemas are strict: unknown keys are rejected with a validation error, so typos like `priorty:` fail loudly instead of being silently ignored. A `version` newer than the current format (`1`) is rejected too — it means the config was written for a newer YMB build.
...
| Key          | Meaning                                                                |
| ------------ | ---------------------------------------------------------------------- |
| `file`       | Required WARNO-relative target path under `GameData/` or `CommonData/` |
| `operations` | Required ordered list of NDF operations applied to that file           |

### Operation Keys

| Key              | Meaning                                                                                       |
| ---------------- | --------------------------------------------------------------------------------------------- |
| `op`             | Required operation kind: `add`, `modify`, `remove`, or `copy`                                 |
| `selector`       | Required target selector that tells YMB what to operate on                                    |
| `value`          | Used by field-path `add`, field-path `modify`, object `add`, and collection-path `add`        |
| `changes`        | Used by object `modify` to rewrite direct fields on the matched object                        |
| `leadingComment` | Optional comment line inserted above generated marker-wrapped content on supported operations |
| `destination`    | Required for `copy`; names where the cloned object should be written                          |
| `position`       | Optional insertion mode for collection-path `add` only                                        |
...
YMB passes one `context` object into that function.

For the builder-provided helper surface, including `context.tools`, see [Script Tools Reference](script-tools.md).
