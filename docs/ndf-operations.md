...
Set `reidentify: true` on a copy to give every literal `DescriptorId = GUID:{...}` in
the copied subtree a distinct, deterministic GUID. This includes anonymous nested objects.
The same source identity, destination name, mod, patch and target file produce the same
GUID on rebuilds and other machines. Other GUID-valued fields and named references are
preserved. Leave it unset when identities are intentionally shared or assigned explicitly.
...
By default only top-level blocks are candidates. Add `recursive: true` to include typed
objects inside fields and collections, including anonymous objects. Match a specific `type`
to isolate siblings. Matching both a parent and its child is rejected to prevent edits from
being applied twice. Fields within each selected object still use the usual nested-field rules.
...
| `equals`      | equals any complete supplied value |
| `notEquals`   | equals none of the supplied values |

For `on: field`, exact comparisons ignore comments and compare parsed scalar values:
`value: '1'` matches `1` and `1.0`, but not `10` or `11`. Quoted strings compare by
their contents. A missing field matches neither `equals` nor `notEquals`. Condition values
are strings (quote numbers in YAML); a value list provides alternatives.
...
| ---------------- | ------------------------- | --------------------------------------- |
| `copy`           | `selector`, `destination` | `leadingComment`, `reidentify`          |
...
| `bulk`           | `match`, `edits`          | `expect`, `leadingComment`, `recursive` |
...
The report states each shared fix and message once, followed by its affected entries.
Operations expanded from one `forEach` share a source location, with each resolved target
listed underneath. Repeated identical entries are counted instead of printed again:

```text
warning  2 patch operations: Collection selector `[TOldModule]` matched no entries. There was nothing to remove. Delete the operation if the game no longer ships this, or fix the selector if it should still match.
           my_pack.units  my_pack/config/patch/units/ymb.patch.yaml:41
             GameData/Generated/Gameplay/Gfx/UniteDescriptor.ndf
               remove Descriptor_Unit_A.ModulesDescriptors[TOldModule]
               remove Descriptor_Unit_B.ModulesDescriptors[TOldModule]
```
...
note     12 marker preview targets: Preview output will not show in-file ownership markers for this file.
           11x Binary output; YMB cannot embed in-file comment markers.
             ...
             ...
           1x This file type does not support YMB comment markers.
             GameData/Localisation/my_pack/INTERFACE.csv
```
