...
> [!TIP]
> Build a preview first. Sync applies it to your WARNO mod folder; keep `.ymb-state` so recovery can restore the originals.
...
> Edit your source mod under `YMB/mods/`, or put local overrides in `ymb.config.yaml`.
> Everything in `.ymb-build` is regenerated, so preview edits are thrown away.
...
The source files in your WARNO mod folder are updated. Run the game's mod-generation
step, then enable that mod in WARNO and try it.
